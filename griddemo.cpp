...
*/
    EVT_TIMER(ID_TIME, GridFrame::OnTimer)
    EVT_GRID_CMD_SELECT_CELL(ID_GRID1, GridFrame::OnSelectCell )
wxEND_EVENT_TABLE()
...
GridFrame::GridFrame()
        : wxFrame( (wxFrame *)NULL, wxID_ANY, "wxWidgets grid class demo",
                   wxDefaultPosition,
                   wxDefaultSize ), m_timer(this, ID_TIME)
{
    ...
    grid = new wxGrid( this,
        ID_GRID1,
                       wxPoint( 0, 0 ),
                       FromDIP(wxSize( 900, 600 )) );

    oGrid = new wxGrid(this,
        ID_GRID2,
        wxPoint(0, 0),
        FromDIP(wxSize(900, 600)));
    ...
    grid->SetFont(wxFont(14, wxFONTFAMILY_SWISS, wxFONTSTYLE_NORMAL, wxFONTWEIGHT_NORMAL, false,"Verdana"));
    grid->EnableGridLines(false);

    oGrid->CreateGrid(0, 0);

    oGrid->GetTable()->SetAttrProvider(new CustomColumnHeadersProvider());

    oGrid->AppendRows(1000);
    oGrid->AppendCols(20);
    oGrid->SetFont(wxFont(14, wxFONTFAMILY_SWISS, wxFONTSTYLE_NORMAL, wxFONTWEIGHT_NORMAL, false, "Verdana"));
    oGrid->EnableGridLines(false);

    ir = oGrid->GetNumberRows();
    oGrid->DeleteRows(0, ir);
    oGrid->AppendRows(ir);
    ...
    struct {
        bool operator()(StockAndOrders a, StockAndOrders b) const {
            if (a.marketValue == 0.0) return false;
            else if (b.marketValue == 0.0) return true;
            else return a.marketValue > b.marketValue;
        }
    } customLarger;

    std::sort(stockProcess.positions.begin(), stockProcess.positions.end(), customLarger);
...
*/
    ...
    gridSizer->Add(grid, wxSizerFlags(1).Expand());
    gridSizer->Add(oGrid, wxSizerFlags(1).Expand());
    topSizer->Add(gridSizer, wxSizerFlags(20).Expand());
...
std::string sCurrency(double c)
{
    char s[16];
    long v = (long)(fabs(c)*100);
    if (v > 100000000) sprintf_s(s, "%d,%03d,%03d.%02d", v / 100000000, v % 100000000 / 100000, v % 100000 / 100, v % 100);
    else if (v > 100000) sprintf_s(s, "%d,%03d.%02d", v / 100000, v % 100000 / 100, v % 100);
    else sprintf_s(s, "%d.%02d", v / 100, v % 100);
    std::string ret;
    if (v < 0) ret = "-";
    ret += s;
    return ret;
...
#define ROWOFEACHSTOCK 5
void GridFrame::OnTimer(wxTimerEvent& event)
{
    stockProcess.process();
    ...
    struct {
        bool operator()(int a, int b) const { 
            bool aFilled = g_pShareOrders[a].filled_qty > 0.1;
            bool bFilled = g_pShareOrders[b].filled_qty > 0.1;

            if (aFilled && !bFilled) return false;
            else if (!aFilled && bFilled) return true;
            else {
                return g_pShareOrders[a].time > g_pShareOrders[b].time;
            }
        }
    } customLarger;

    std::sort(stockProcess.allOrders.begin(), stockProcess.allOrders.end(), customLarger);

    wxColour green(0, 128, 64);
    wxColour yellow(0xef, 0xee, 0xd2);
    wxColour lightBlack(25, 25, 25);
    wxColour darkblue(0x0f, 0x0f, 0x3b);
    wxColour darkred(0x87, 0x00, 0x00);

    int row = 0;
    for (auto pos = stockProcess.positions.begin(); pos != stockProcess.positions.end(); pos++) {
        int stockrow = row;
        grid->SetCellValue(row, 0, pos->stock->symbol);
        grid->SetCellTextColour(row, 0, *wxBLACK);
        grid->SetCellBackgroundColour(row, 0, yellow);
//        grid->SetCellValue(row, 1, sFormat("%6d", pos->stock->bidSize));
        grid->SetCellValue(row, 1, sFormat("%8.2lf", pos->stock->bottomPrice));
        grid->SetCellTextColour(row, 1, *wxWHITE);
        grid->SetCellBackgroundColour(row, 1, row%2?*wxBLACK: darkred);
        grid->SetCellValue(row, 2, sFormat("%8.2lf", pos->stock->bidPrice));
        grid->SetCellTextColour(row, 2, *wxWHITE);
        grid->SetCellBackgroundColour(row, 2, row%2?*wxBLACK:lightBlack);
        grid->SetCellValue(row, 3, sFormat("%8.2lf", pos->stock->askPrice));
        grid->SetCellTextColour(row,3, *wxWHITE);
        grid->SetCellBackgroundColour(row, 3, row%2?*wxBLACK:lightBlack);
        grid->SetCellValue(row, 4, sFormat("%8.2lf", pos->stock->topPrice));
        grid->SetCellTextColour(row, 4, *wxWHITE);
        grid->SetCellBackgroundColour(row, 4, row%2?*wxBLACK: darkred);
        grid->SetCellValue(row, 5, sFormat("%8.2lf", pos->stock->lastPrice));
        grid->SetCellTextColour(row, 5, *wxWHITE);
        grid->SetCellBackgroundColour(row, 5, row%2?*wxBLACK:lightBlack);
        if (pos->stock->closePrice == 0.0) pos->stock->closePrice = pos->stock->previousClose;
        grid->SetCellValue(row, 6, sFormat("%8.2lf%%", (pos->stock->lastPrice / pos->stock->closePrice - 1) * 100));
        grid->SetCellTextColour(row, 6, *wxWHITE);
        grid->SetCellBackgroundColour(row, 6, pos->stock->lastPrice - pos->stock->closePrice <0?*wxRED:green);
        grid->SetCellValue(row, 7, sFormat("%d", int(pos->IBpos+pos->TDpos)));
        grid->SetCellTextColour(row, 7, *wxWHITE);
        grid->SetCellBackgroundColour(row, 7, pos->IBpos + pos->TDpos < 0 ? *wxRED : green);
        grid->SetCellValue(row, 8, sFormat("%d:%d", int(pos->IBpos), int(pos->TDpos)));

        grid->SetCellValue(row, 9, sCurrency(pos->marketValue));
        grid->SetCellTextColour(row, 9, *wxWHITE);
        grid->SetCellBackgroundColour(row, 9, pos->marketValue < 0 ? *wxRED : green);

        std::sort(pos->orders.begin(), pos->orders.end(), customLarger);
        int count = 0;
        for (auto o : pos->orders) {
            row++;
            SetOrderRow(grid, row, o);
            grid->SetCellValue(row, 0, "");

            count++;
            if (count >= ROWOFEACHSTOCK) break;
        }

        for (; count < ROWOFEACHSTOCK; count++) {
            row++;
    ...
    if (selectedStockIndex == -1) {
        int row = 0;
        for (auto o : stockProcess.allOrders) {
            if (g_pShareOrders[o].filled_qty < 0.1) continue;
            SetOrderRow(oGrid, row, o);
            tProfit += g_pShareOrders[o].profit;
            tCommission += g_pShareOrders[o].commission;
            tCash += (g_pShareOrders[o].action[0] == 'B'?-1:1)*g_pShareOrders[o].filled_qty * g_pShareOrders[o].price;
            row++;
        }
        nOrders = row;
    }
    else if (selectedStockIndex < stockProcess.positions.size()) {
        int row = 0;
        for (auto o : stockProcess.positions[selectedStockIndex].orders) {
            if (g_pShareOrders[o].filled_qty < 0.1) continue;
            SetOrderRow(oGrid, row, o);
            tProfit += g_pShareOrders[o].profit;
            tCommission += g_pShareOrders[o].commission;
        ...
        nOrders = row;
        for (; row < stockProcess.allOrders.size(); row++) {
            oGrid->SetCellBackgroundColour(row, 1, *wxWHITE);
            oGrid->SetCellBackgroundColour(row, 5, *wxWHITE);
        }
    }
    if (stockProcess.positions.size() == 0) {
        oGrid->ClearGrid();
        oGrid->ClearBackground();
    }

    char output[1000];
    sprintf_s(output, "%d Orders. Total Cash Flow: %g, Total Profit: %8.2lf (%4.2lf%%) Total Commission: %8.2lf\n", nOrders, tCash, tProfit, tProfit/tCash*100, tCommission);
    std::string sOut = output;
    if (selectedStockIndex < stockProcess.positions.size()) {
        WangStock* p = stockProcess.positions[selectedStockIndex].stock;
        double expProfit = 0.5 * p->multiplier * 100 / log(p->topPrice / p->bottomPrice) * p->previousClose * p->stdVariation * p->stdVariation;
        sprintf_s(output, "Top: %g, Bottom: %g, Multiplier: %g, Offset: %g, Previous Close %g, StdVariation %g%%, lowBound %g%%(%g%%), Expected Profit: %g (realized %5.1lf%%)\n", 
            p->topPrice, p->bottomPrice, p->multiplier, p->offset, p->previousClose, p->stdVariation*100, -p->stdVariation*(1+p->lowBoundDiscount) * 100, p->lowBoundDiscount*100, 
            expProfit, 100*tProfit/expProfit);
        sOut += output;
    }
    logWin->SetValue(sOut);
}
...
void GridFrame::SetOrderRow(wxGrid* g, int row, int o)
{
    wxColour green(0, 128, 64);
    wxColour yellow(0xef, 0xee, 0xd2);
    wxColour darkyellow(0xdf, 0xde, 0xc2);
    wxColour lightBlack(25, 25, 25);
    g->SetCellValue(row, 0, g_pShareOrders[o].sym);
    g->SetCellTextColour(row, 0, *wxBLACK);
    if (g_pShareOrders[o].status[0] == 'F' || g_pShareOrders[o].status[0] == 'C') g->SetCellBackgroundColour(row, 0, yellow);
    else g->SetCellBackgroundColour(row, 0, darkyellow);
    g->SetCellValue(row, 1, g_pShareOrders[o].action);
    g->SetCellTextColour(row, 1, *wxWHITE);
    if (g_pShareOrders[o].action[0] == 'B') g->SetCellBackgroundColour(row, 1, *wxBLUE);
    else g->SetCellBackgroundColour(row, 1, *wxRED);
    g->SetCellValue(row, 2, sFormat("%8.0lf", g_pShareOrders[o].qty));
    g->SetCellTextColour(row, 2, *wxWHITE);
    g->SetCellBackgroundColour(row, 2, row%2?*wxBLACK:lightBlack);
    g->SetCellValue(row, 3, sFormat("%8.2lf", g_pShareOrders[o].price));
    g->SetCellTextColour(row, 3, *wxWHITE);
    g->SetCellBackgroundColour(row, 3, row%2?*wxBLACK:lightBlack);
    g->SetCellValue(row, 4, sFormat("%8.0lf", g_pShareOrders[o].filled_qty));
    g->SetCellTextColour(row, 4, *wxWHITE);
    g->SetCellBackgroundColour(row, 4, row%2?*wxBLACK:lightBlack);
    g->SetCellValue(row, 5, g_pShareOrders[o].status);
    g->SetCellTextColour(row, 5, *wxWHITE);
    g->SetCellBackgroundColour(row, 5, g_pShareOrders[o].status[0] != 'F' ? *wxRED : green);
    g->SetCellValue(row, 6, sFormat("%6.2lf", g_pShareOrders[o].profit));
    if (fabs(g_pShareOrders[o].commission) > 1000) g_pShareOrders[o].commission = 0;
    g->SetCellValue(row, 7, sFormat("%5.2lf", g_pShareOrders[o].commission));

    struct tm* tm_struct = localtime(&g_pShareOrders[o].time);
    g->SetCellValue(row, 8, sFormat("%02d:%02d:%02d", tm_struct->tm_hour, tm_struct->tm_min, tm_struct->tm_sec));
    g->SetCellValue(row, 9, g_pShareOrders[o].broker);
    g->SetCellValue(row, 10, g_pShareOrders[o].id);
...
void GridFrame::OnSelectCell( wxGridEvent& ev )
{
    ...
    logBuf << "cell at row " << ev.GetRow()
           << " col " << ev.GetCol() << "pos " << ev.GetPosition().x << ", " << ev.GetPosition().y 
    ...
    */
    ...
    if (ev.GetRow() % (ROWOFEACHSTOCK+1) != 0) selectedStockIndex = -1;
    else selectedStockIndex = ev.GetRow() / (ROWOFEACHSTOCK + 1);
    ...
    if (selectedStockIndex == -1) {
        int row = 0;
        for (auto o : stockProcess.allOrders) {
            if (g_pShareOrders[o].profit < 0.01 && g_pShareOrders[o].filled_qty < 0.1) continue;
            SetOrderRow(oGrid, row, o);
            tProfit += g_pShareOrders[o].profit;
            tCommission += g_pShareOrders[o].commission;
            tCash += (g_pShareOrders[o].action[0] == 'B' ? -1 : 1) * g_pShareOrders[o].filled_qty * g_pShareOrders[o].price;
    ...
    if (selectedStockIndex < stockProcess.positions.size()) {
        WangStock* p = stockProcess.positions[selectedStockIndex].stock;
        sprintf_s(output, "Top: %g, Bottom: %g, Multiplier: %g, Offset: %g\n", p->topPrice, p->bottomPrice, p->multiplier, p->offset);
        sOut += output;
    }
    logWin->SetValue(output);
